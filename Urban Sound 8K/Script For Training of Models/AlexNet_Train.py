...
class Teacher(nn.Module): # Subband-1 Network using Pre-Trained Resent-34
    def __init__(self):
        ...
        Pre_Trained_Layers=nn.Sequential(*list(models.alexnet(pretrained=True).children())[:-1])
...
def evaluate(model,device,iterator, criterion): # Evaluate Validation accuracy 
    ...
    return epoch_loss / len(iterator), epoch_acc / len(iterator) 
MODEL_SAVE_PATH = os.path.join("/home/mani/Desktop/AK/Empirical Study of Adversarial Examples for ESC/Urban8k/Trained Models", 'AlexNet_CNN.pt') # Define Path to save the model 
...
np.save('/home/mani/Desktop/AK/Empirical Study of Adversarial Examples for ESC/Urban8k/Model Parameters/AlexNet_CNN_Parameters',Temp) # Save Temp Array as numpy array 
